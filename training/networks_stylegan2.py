...
#----------------------------------------------------------------------------
# Main generator network. Uses smaller version of G_synthesis for 128x128 as in PGGAN.
# Composed of two sub-networks (mapping and synthesis) that are defined below.
# Used in configs B-F (Table 1).
...
def G_main_small(
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
def G_main_small_sh(
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_sh',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
def G_main_small_sh_lr(
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_sh_lr',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
#----------------------------------------------------------------------------
# Main generator network. Uses smaller version of G_synthesis for 128x128 as in PGGAN.
# Multiple learnable FSGs.
# Composed of two sub-networks (mapping and synthesis) that are defined below.
# Used in configs B-F (Table 1).

def G_main_small_fsg_lr(
    latents_in,                                         # First input: Latent vectors (Z) [minibatch, latent_size].
    labels_in,                                          # Second input: Conditioning labels [minibatch, label_size].
    truncation_psi          = 0.5,                      # Style strength multiplier for the truncation trick. None = disable.
    truncation_cutoff       = None,                     # Number of layers for which to apply the truncation trick. None = disable.
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_fsg_lr',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
#----------------------------------------------------------------------------
# Main generator network. Uses smaller version of G_synthesis for 128x128 as in PGGAN.
# Multiple learnable FSGs with full resolution branches.
# Composed of two sub-networks (mapping and synthesis) that are defined below.
# Used in configs B-F (Table 1).

def G_main_small_fsg_fellres_lr(
    latents_in,                                         # First input: Latent vectors (Z) [minibatch, latent_size].
    labels_in,                                          # Second input: Conditioning labels [minibatch, label_size].
    truncation_psi          = 0.5,                      # Style strength multiplier for the truncation trick. None = disable.
    truncation_cutoff       = None,                     # Number of layers for which to apply the truncation trick. None = disable.
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_fsg_fullres_lr',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
def G_main_small_fsg(
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_fsg',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
def G_main_small_fsg_noshift(
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_fsg_noshift',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
def G_main_small_fsg_noshare(
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_fsg_noshare',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
def G_main_small_fsg_noshare_jointcomplex(
    latents_in,                                         # First input: Latent vectors (Z) [minibatch, latent_size].
    labels_in,                                          # Second input: Conditioning labels [minibatch, label_size].
    truncation_psi          = 0.5,                      # Style strength multiplier for the truncation trick. None = disable.
    truncation_cutoff       = None,                     # Number of layers for which to apply the truncation trick. None = disable.
    ...
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2_small_fsg_noshare_jointcomplex',  # Build func name for the synthesis network.
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
...
def G_synthesis_stylegan2_small(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
...
def G_synthesis_stylegan2_small_sh(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
...
#----------------------------------------------------------------------------
# StyleGAN2 synthesis network (Figure 7). Smaller version for 128x128 as in PGGAN.
# Output shifted generator with learnable target frequency.
# Implements skip connections and residual nets (Figure 7), but no progressive growing.
# Used in configs E-F (Table 1).

def G_synthesis_stylegan2_small_sh_lr(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def freq_shift_gen(gr, gi, fc_x, fc_y):
        with tf.variable_scope('Shift'):
            ...
            fc_x_norm += tf.stop_gradient(fc_x * (1 - freq_norm))
            fc_y_norm = tf.tile(tf.reshape(fc_y, [1]), [im_size]) * freq_norm
            fc_y_norm += tf.stop_gradient(fc_y * (1 - freq_norm))

            ### build cosine and sine kernels
            kernel_loc = tf.convert_to_tensor(np.arange(im_size), dtype=tf.float32)
            kernel_input = tf.reshape(2. * np.pi * fc_x_norm * kernel_loc, [1, 1, im_size]) + \
                tf.reshape(2. * np.pi * fc_y_norm * kernel_loc, [1, im_size, 1])
            kernel_cos = tf.cos(kernel_input)
            kernel_sin = tf.sin(kernel_input)
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            ...
            if res == resolution_log2:
                with tf.variable_scope('FSG'):
                    gi = torgb(block(x, res), y, res)
            x = block(x, res)
            if architecture == 'skip' or res == resolution_log2:
                y = torgb(x, y, res)
    ...
    with tf.variable_scope('Shift'):
        freq_lr = 1.
        with tf.variable_scope('TargetFreqs'):
            target_freqs = tf.get_variable('targetfreqs', shape=[1, 2], 
                initializer=tf.initializers.random_normal(0, 1./8 /freq_lr)) * freq_lr
        images_out = freq_shift_gen(y, gi, target_freqs[0, 0], target_freqs[0, 1])
...
#----------------------------------------------------------------------------
# Frequency Shifted Generators with learnable target frequencies.
# StyleGAN2 synthesis network (Figure 7). Smaller version for 128x128 as in PGGAN.
# Implements skip connections and residual nets (Figure 7), but no progressive growing.
# Used in configs E-F (Table 1).

def G_synthesis_stylegan2_small_fsg_lr(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            ...
            if res == resolution_log2 - 1:
                ...
                with tf.variable_scope('TargetFreqs'):
                    target_freqs = tf.get_variable('targetfreqs', shape=[freq_num, 2], 
                        initializer=tf.initializers.random_normal(0, 1./8 /freq_lr)) * freq_lr
                ...
                for i in range(2*freq_num):
                    ...
                    g_list.append(g)
                for i in range(freq_num):
                    fsg = freq_shift_gen(g_list[i*2], g_list[i*2+1], target_freqs[i, 0], target_freqs[i, 1])
                    fsg_list.append(fsg)
...
#----------------------------------------------------------------------------
# Frequency Shifted Generators with learnable target frequencies and full resolution branches.
# StyleGAN2 synthesis network (Figure 7). Smaller version for 128x128 as in PGGAN.
# Implements skip connections and residual nets (Figure 7), but no progressive growing.
# Used in configs E-F (Table 1).

def G_synthesis_stylegan2_small_fsg_fullres_lr(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
                ...
                for i in range(2*freq_num):
                    with tf.variable_scope('FSG_{}'.format(i)):
                        with tf.variable_scope(f'{2**res}x{2**res}'):
                            g = block(x, res, resolution_log2)
                        with tf.variable_scope(f'{2**(res+1)}x{2**(res+1)}'):
                            g = torgb(block(g, res+1, resolution_log2), None, resolution_log2)
                    g_list.append(g)
                for i in range(freq_num):
                    fsg = freq_shift_gen(g_list[i*2], g_list[i*2+1], target_freqs[i, 0], target_freqs[i, 1])
                    fsg_list.append(fsg)
...
def G_synthesis_stylegan2_small_fsg(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def freq_shift_gen(gr, gi, fc_x, fc_y):
        with tf.variable_scope('Shift'):
            ...
            tfkernel_cos = tf.convert_to_tensor(kernel_cos, dtype=tf.float32)
            tfkernel_sin = tf.convert_to_tensor(kernel_sin, dtype=tf.float32)
            return gr * tfkernel_cos - gi * tfkernel_sin if np.any(kernel_sin > 0.) else gr * tfkernel_cos
...
def G_synthesis_stylegan2_small_fsg_noshift(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
...
def G_synthesis_stylegan2_small_fsg_noshare(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    fsg_list = list()
    g_list = list()
    for fi in range(2*len(freq_list)):
        with tf.variable_scope(f'ShiftedG_{fi}'):
            # Early layers.
            y = None
            with tf.variable_scope('4x4'):
                with tf.variable_scope('Const'):
            ...
            for res in range(3, resolution_log2):
                with tf.variable_scope('%dx%d' % (2**res, 2**res)):
                    x = block(x, res, resolution_log2)
                    if architecture == 'skip':
                        y = upsample(y)
                    if architecture == 'skip' or res == resolution_log2-1:
                        y = torgb(x, y, resolution_log2)
            g_list.append(y)

    for i, fc in enumerate(freq_list):
        fsg = freq_shift_gen(upsample(g_list[i*2]), upsample(g_list[i*2+1]), fc[0], fc[1])
        fsg_list.append(fsg)
    
    with tf.variable_scope('Add_FSG'):
        images_out = images_out + sum(fsg_list) if len(fsg_list) > 0 else images_out
     ### end of frequency shifted gens
...
def G_synthesis_stylegan2_small_fsg_noshare_jointcomplex(
    dlatents_in,                        # Input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
            ...
            for res in range(3, resolution_log2):
                with tf.variable_scope('%dx%d' % (2**res, 2**res)):
                    ...
                    if architecture == 'skip':
                        y = upsample(y)
                    if res == resolution_log2-1:
                        y_complex = torgb(x, y, resolution_log2)
                        y = torgb(x, y, resolution_log2)
                    elif architecture == 'skip':
                        y = torgb(x, y, resolution_log2)

            g_list.append(y)
            g_list.append(y_complex)
...
#----------------------------------------------------------------------------
# StyleGAN2 discriminator (Figure 7). Smaller version for 128x128 as in PGGAN.
# Implements skip connections and residual nets (Figure 7), but no progressive growing.
# Used in configs E-F (Table 1).

def D_stylegan2_small(
    images_in,                          # First input: Images [minibatch, channel, height, width].
    labels_in,                          # Second input: Labels [minibatch, label_size].
    num_channels        = 3,            # Number of input color channels. Overridden based on dataset.
    resolution          = 1024,         # Input resolution. Overridden based on dataset.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def block(x, res): # res = 2..resolution_log2
        ...
        with tf.variable_scope('Conv1_down'):
            x = apply_bias_act(conv2d_layer(x, fmaps=nf(nf_res-2), kernel=3, down=True, resample_kernel=resample_kernel), act=act)
        if architecture == 'resnet':
            with tf.variable_scope('Skip'):
                t = conv2d_layer(t, fmaps=nf(nf_res-2), kernel=1, down=True, resample_kernel=resample_kernel)
                x = (x + t) * (1 / np.sqrt(2))
        return x
    def downsample(y):
        with tf.variable_scope('Downsample'):
